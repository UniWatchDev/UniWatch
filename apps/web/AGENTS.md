...
- `src/app/page.tsx` — Server Component: Magazine-editorial layout (masthead → hero + by-the-numbers stats → endpoint explorer + letters → health + pitch footer).
- `src/app/health-check.tsx` — Client Component: auto-pings `healthContract`, renders dot + latency.
- `src/app/notes-panel.tsx` — Client Component: full CRUD styled as a "Letters to the Editor" column.
