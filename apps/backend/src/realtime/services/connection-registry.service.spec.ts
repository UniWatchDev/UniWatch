...
describe('ConnectionRegistryService', () => {
  ...
  it('resolves the user behind a registered socket', () => {
    registry.register('socket-1', { userId: 'user-1', userName: 'Ada', avatarId: 'violet-reel' });

    expect(registry.getUser('socket-1')).toEqual({ userId: 'user-1', userName: 'Ada', avatarId: 'violet-reel' });
    expect(registry.getSocketIds('user-1')).toEqual(['socket-1']);
  });

  it('tracks multiple concurrent sockets for the same user', () => {
    registry.register('socket-1', { userId: 'user-1', userName: 'Ada', avatarId: 'violet-reel' });
    registry.register('socket-2', { userId: 'user-1', userName: 'Ada', avatarId: 'violet-reel' });
  ...
  it('removes only the unregistered socket and keeps the others', () => {
    ...
    expect(removed).toEqual({ userId: 'user-1', userName: 'Ada', avatarId: 'violet-reel' });
