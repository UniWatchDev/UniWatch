...
- `src/mail/` — `MailModule` + `MailService`: **Resend** over SMTP (`nodemailer`); env template in `.env.*.example`. `503` if send fails.
- `src/auth/` — `AuthModule` + `AuthController` + `AuthService`: register / resend / forgot send email when SMTP is configured (`503` if send fails); verification codes and reset tokens are **never** returned in JSON (email-only). `POST /auth/verify-email`, refresh (rotating refresh cookie), logout, `GET /me` behind `JwtAuthGuard` (access JWT includes `pv`), Zod DTOs in `auth.dto.ts`. E2e tests override `MailService` to capture outbound codes/tokens without SMTP.
