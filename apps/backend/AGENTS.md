...
- `src/auth/` — `AuthModule` + `AuthController` + `AuthService`: register (issues email verification code; optional `debug` when `AUTH_DEBUG_EMAIL_TOKENS`), `POST /auth/verify-email`, `POST /auth/resend-verification` (non-enumerating `202`), login **blocked until `emailVerified`**, refresh (rotating refresh cookie), logout, `GET /me` behind `JwtAuthGuard`, Zod DTOs in `auth.dto.ts`.
