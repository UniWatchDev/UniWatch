...
- `src/utils/env.validation.ts` — Zod schema validating `NODE_ENV`, `PORT`, `CORS_ORIGIN`, `MONGODB_URI`, JWT, auth TTLs, `AUTH_USE_REAL_EMAILS`, optional SMTP (`SMTP_HOST`, `SMTP_PORT`, …), optional `APP_PUBLIC_ORIGIN` for reset links, optional **`AUTH_DEBUG_LOG`**, and **`AUTH_THROTTLE_TTL_MS` / `AUTH_THROTTLE_LIMIT`** for auth route rate limiting.
