...
- `src/utils/env.validation.ts` — Zod schema validating `NODE_ENV`, `PORT`, `CORS_ORIGIN`, JWT, auth TTLs, `AUTH_USE_REAL_EMAILS`, optional SMTP (`SMTP_HOST`, `SMTP_PORT`, …), and optional `APP_PUBLIC_ORIGIN` for reset links.
...
- `src/auth/` — `AuthModule` + `AuthController` + `AuthService`: register / resend / forgot always return verification/reset codes in JSON; real Resend delivery only runs when `AUTH_USE_REAL_EMAILS=true` (`503` if send fails). `POST /auth/verify-email`, refresh (rotating refresh cookie), logout, `POST /auth/change-password` (session + current password), `GET /me` behind `JwtAuthGuard` (access JWT includes `pv`), Zod DTOs in `auth.dto.ts`.
