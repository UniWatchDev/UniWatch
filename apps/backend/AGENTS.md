...
- `src/utils/env.validation.ts` — Zod schema validating `NODE_ENV`, `PORT`, `CORS_ORIGIN`, and JWT-related env at startup.
...
- `src/auth/` — `AuthModule` + `AuthController` + `AuthService`: register (issues email verification code; optional `debug` when `AUTH_DEBUG_EMAIL_TOKENS`), `POST /auth/verify-email`, `POST /auth/resend-verification` and `POST /auth/forgot-password` (non-enumerating `202`; optional reset `debug` with same flag), `POST /auth/reset-password` (`204`, revokes refresh + bumps `passwordVersion`), login **blocked until `emailVerified`**, refresh (rotating refresh cookie), logout, `GET /me` behind `JwtAuthGuard` (access JWT includes `pv` / password version), Zod DTOs in `auth.dto.ts`.
