...
- `src/auth/` — `AuthModule` + `AuthController` + `AuthService`: register, login, refresh (rotating refresh cookie), logout, `GET /me` behind `JwtAuthGuard`, Zod DTOs in `auth.dto.ts`.
