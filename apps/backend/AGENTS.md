...
- `src/auth/` — `AuthModule` + `AuthController` + `AuthService`: register / resend / forgot always return verification/reset codes in JSON; real Resend delivery only runs when `AUTH_USE_REAL_EMAILS=true` (`503` if send fails). `POST /auth/verify-email`, refresh (rotating refresh cookie), logout, `GET /me` behind `JwtAuthGuard` (access JWT includes `pv`), Zod DTOs in `auth.dto.ts`.
