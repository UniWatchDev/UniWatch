...
- `src/main.ts` — entrypoint: creates the Nest app, calls `configureApp()`, listens on `PORT`.
- `src/bootstrap.ts` — exports `configureApp(app, configService)`: applies `helmet()` with its full default **strict CSP**, `setGlobalPrefix('api')`, `enableCors`, and mounts Swagger at `/docs` **only when `NODE_ENV !== 'production'`** (Swagger ships inline scripts that would break under CSP, so it's gated rather than opting out of CSP). Shared between `main.ts` and the e2e suite so tests run against the same configured app.
...
- `src/utils/env.validation.ts` — Zod schema validating `NODE_ENV` + `PORT` + `CORS_ORIGIN` at startup.
...
- `src/notes/` — full CRUD at `/api/notes` with in-memory `Map` storage. `:id` routes use `NoteIdParamsDto` (Zod-backed) instead of `ParseUUIDPipe`.
