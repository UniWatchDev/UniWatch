...
- `apps/backend` — NestJS 11 API under `/api` prefix (port 3000 via `PORT` env, Swagger UI at `/docs`). Loads `apps/backend/.env.${NODE_ENV}` only; copy `env.development.template` / `env.production.template` into gitignored `.env.development` / `.env.production` when bootstrapping. Movie files use **Cloudflare R2** (`uniwatch-dev` / `uniwatch-production` buckets via `S3_*` env). Large videos use a **backend relay HLS pipeline**: the browser posts the file body to `/api/movies/:id/upload`, the backend streams it into R2 and ffmpeg at the same time, publishes growing playlists/segments to R2 (`R2_PUBLIC_BASE_URL`), and emits `room:video-ready` once the full ladder is ready — keeping room creation/upload non-blocking. Auth and **`/api/notes`**, **`/api/movies`**, **`/api/rooms`** require a logged-in session (HttpOnly JWT cookies; use `credentials: 'include'` from browsers). Notes and movies are **scoped by `ownerId`** (JWT `sub`); room **`creator`** is set server-side from the JWT; **`POST /api/auth/*`** is rate-limited per IP (see `ThrottlerModule` + env `AUTH_THROTTLE_*`).
- `apps/mobile` — empty placeholder

### Shared packages

- `@repo/consts` — string constants: `API_BASE_URL`, endpoint paths (`/api`, `/api/auth/*` including register/login/refresh/me/logout/verify/resend/forgot/reset, `/api/health`, `/api/notes`, `/api/notes/:id`, `/api/movies`, `/api/movies/resolve`, `/api/movies/:id`, `/api/movies/:id/upload`, `/api/movies/:id/stream`, `/api/movies/:id/presign-upload`, `/api/movies/:id/complete-upload`), starter copy, and **`/realtime`** Socket.IO event names (`REALTIME_CLIENT_EVENTS` / `REALTIME_SERVER_EVENTS`, including the async video lifecycle `room:video-processing` / `room:video-playable` / `room:video-ready` / `room:video-failed`) shared by the gateway and frontend client. Leaf package, no internal deps.
- `@repo/schemas` — Zod 4 schemas and inferred types. Subpaths: `/auth`, `/health`, `/notes`, `/movies`, `/rooms`, `/realtime`, `/root`, `/errors` (RFC 7807 `problemDetailsSchema`). Notes exports include `noteIdParamsSchema` for UUID path params. Movies include upload/stream response shapes, relaxed create schema (`name` + `language` required), the extended `movieUploadStatusSchema` (`pending|uploading|processing|ready|failed`), HLS fields on `movieResponseSchema` (`hls_prefix`, `playback_url`, `playback_partial`, `available_qualities`, `error_message`), and the direct-upload shapes (`presignUploadRequest/Response`, `completeUploadRequest/Response`). Realtime exports room-state, chat, playback, join/leave payload schemas (`connectedUserSchema` keyed by `userId` with a `socketIds` array; `roomStatusSchema` re-exported from `/rooms`), `roomClosedEventSchema` for host-initiated room teardown, plus the async video lifecycle schemas (`videoProcessingEventSchema`, `videoPlayableEventSchema`, `videoReadyEventSchema`, `videoFailedEventSchema`).
- `@repo/contracts` — typed `EndpointContract<TResponse, TBody, TParams, TQuery>` objects. Each contract attaches `responseSchema` plus any of `bodySchema`/`paramsSchema`/`querySchema` as real Zod schemas from `@repo/schemas`. Subpaths: `/auth`, `/health`, `/notes`, `/movies` (incl. `presignUploadContract`, `completeUploadContract`), `/root`, and **`/realtime`** (`SocketEventContract<TPayload>` tying each event name to its payload schema + direction, incl. the video lifecycle contracts). Depends on `@repo/consts`, `@repo/schemas`, `zod`.
